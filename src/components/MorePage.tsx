...
import { LogOut, User as UserIcon, Bell, ChevronRight, type LucideIcon } from 'lucide-react';
...
export default function MorePage({ onNavigate, onLogout }: { onNavigate: (id: string) => void; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-full bg-slate-50">
      <button onClick={() => onNavigate('account')} className="w-full bg-white px-5 py-5 border-b border-slate-100 flex items-center gap-3 text-left active:bg-slate-50">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-slate-400">Hesap ayarlarını yönet</div>
        </div>
        <ChevronRight className="w-5 h-5 text-slate-300 shrink-0" />
      </button>

      {/* Hızlı: Hesabım · Bildirimler */}
      <div className="px-4 pt-5">
        <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-1 mb-2">Hesap</div>
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden divide-y divide-slate-50">
          <button onClick={() => onNavigate('account')} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 active:bg-slate-100 transition-colors text-left">
            <span className="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center shrink-0"><UserIcon className="w-[18px] h-[18px]" /></span>
            <span className="text-sm font-medium text-slate-700">Hesabım</span>
            <span className="ml-auto text-slate-300">›</span>
          </button>
          <button onClick={() => onNavigate('notifications')} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 active:bg-slate-100 transition-colors text-left">
            <span className="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center shrink-0"><Bell className="w-[18px] h-[18px]" /></span>
      ...
      {NAV_GROUPS.map((g) => {
        const items = g.items.filter((it) => it.id !== 'dashboard' && it.id !== 'notifications' && (!it.perm || canAny(...it.perm)));
